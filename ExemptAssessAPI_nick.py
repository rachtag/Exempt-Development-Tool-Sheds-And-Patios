...
def patio_check():
    """ This function provides the rule-set for Exempt Development of a Patio under the SEPP
        ...
        a string indicating reason for Not Exempt under SEPP"""
    
    print("\n🪴 Patio Development Check\n")
    issues = []

    if attributes["zoning"] not in ["R1", "R2", "R3", "R4", "R5", "RU1", "RU2", "RU3", "RU4", "RU6"]:
        issues.append("Our tool currently does not support your zone. Please contact Albury City Council for further assistance.")

    if attributes["structure_type"] == "replacement":
        if attributes["height_existing"] > 1:
            issues.append("❌ Not Exempt for development as the proposed structure is higher than 1m above ground level. Please refer to the SEPP legislation for height restrictions: https://legislation.nsw.gov.au/view/html/inforce/current/epi-2008-0572#sec.2.11")

        if attributes["material_quality"] == "no":
            issues.append("❌ Not Exempt for development as the replacement must use materials of equal or better quality. Please refer to the SEPP legislation for material quality restrictions: https://legislation.nsw.gov.au/view/html/inforce/current/epi-2008-0572#sec.2.12")
    
        if attributes["same_size"] == "no":
            issues.append("❌ Not Exempt for development as the replacement must be the same size and height as the existing structure. Please refer to the SEPP legislation for size and height restrictions: https://legislation.nsw.gov.au/view/html/inforce/current/epi-2008-0572#sec.2.12")

    if attributes["heritage"] == "yes":
        issues.append("❌ Not Exempt for development because the property is a heritage item. Please refer to the SEPP legislation for heritage item restrictions: https://legislation.nsw.gov.au/view/html/inforce/current/epi-2008-0572#sec.2.11")

    if attributes["foreshore"] == "yes":
        issues.append("❌ Not Exempt for development because the property is located in a foreshore area. Please refer to the SEPP legislation for foreshore area restrictions: https://legislation.nsw.gov.au/view/html/inforce/current/epi-2008-0572#sec.2.11")

    if attributes["area"] > 25:
        issues.append("❌ Not Exempt for development as the structure area is more than 25 m². Please refer to the SEPP legislation for area restrictions: https://legislation.nsw.gov.au/view/html/inforce/current/epi-2008-0572#sec.2.12")
    else:
        if attributes["land_size"] > 300:
            if attributes["total_structures_area"] > 0.15 * attributes["land_size"]:
                issues.append("❌ Not Exempt for development as the combined area of these structures is over the limit for this lot size. Please refer to the SEPP legislation for total area restrictions: https://legislation.nsw.gov.au/view/html/inforce/current/epi-2008-0572#sec.2.12")
        else:
            if attributes["total_structures_area"] > 25:
                issues.append("❌ Not Exempt for development as the combined area of these structures is over the limit for this lot size. Please refer to the SEPP legislation for total area restrictions: https://legislation.nsw.gov.au/view/html/inforce/current/epi-2008-0572#sec.2.12")

    if attributes["wall_height"] == "yes":
        issues.append("❌ Not Exempt for development as the wall height exceeds 1.4m. Please refer to the SEPP legislation for wall height restrictions: https://legislation.nsw.gov.au/view/html/inforce/current/epi-2008-0572#sec.2.12")
    
    ## Subdivision 6, 2.12 (1)(e)(i) missing? if carried out in connection with farm experience premises or 
    ## farm gate premises—more than 50m from a road 
    # if attributes["road_distance_ls50"] =="no" :
    #     issues.append("❌ Not Exempt for development as the distance should be over 50 metres from a road. Restriction: SEPP 2.12(1)(e)(i) https://legislation.nsw.gov.au/view/html/inforce/current/epi-2008-0572#sec.2.12")

    if attributes["behind_building_line"] == "no":
        issues.append("❌ Not Exempt for development as the structure must be behind the front building line. Please refer to the SEPP legislation for building line restrictions: https://legislation.nsw.gov.au/view/html/inforce/current/epi-2008-0572#sec.2.12")

    if attributes["boundary_distance"] < 900:
        issues.append("❌ Not Exempt for development as the structure must be at least 900mm from the boundary. Please refer to the SEPP legislation for boundary distance restrictions: https://legislation.nsw.gov.au/view/html/inforce/current/epi-2008-0572#sec.2.12")
    elif attributes["boundary_distance"] < 5000:
        if attributes["zoning"] in ["R5", "RU1", "RU2", "RU3", "RU4", "RU6"]:
            issues.append("❌ Not Exempt for development as the structure must at least 5m from the boundary. Please refer to the SEPP legislation for boundary distance restrictions: https://legislation.nsw.gov.au/view/html/inforce/current/epi-2008-0572#sec.2.12")
    
    ## Need comment on front page that this condition does not apply to development carried out in connection with a building used for 
    ## the purposes of farm stay accommodation, farm gate premises or farm experience premises as description Subdivision 6, 2.12(3)
    if attributes["metal"] == "yes":        
        if attributes["reflective"] == "no": 
            issues.append("❌ Not Exempt for development as metal components must be low-reflective and factory pre-coloured. Please refer to the SEPP legislation for metal component restrictions: https://legislation.nsw.gov.au/view/html/inforce/current/epi-2008-0572#sec.2.12")

    if attributes["floor_height"] > 1000:
        issues.append("❌ Not Exempt for development as the floor height exceeds 1m above ground level. Please refer to the SEPP legislation for floor height restrictions: https://legislation.nsw.gov.au/view/html/inforce/current/epi-2008-0572#sec.2.12")

    if attributes["roof"] == "yes":
        if attributes["overhang"] > 600:
            issues.append("❌ Not Exempt for development as the roof overhang exceeds 600mm. Please refer to the SEPP legislation for roof overhang restrictions: https://legislation.nsw.gov.au/view/html/inforce/current/epi-2008-0572#sec.2.12")

        if attributes["attached"] == "yes":
            if attributes["above_gutter"] == "yes":
                issues.append("❌ Not Exempt for development as the roof must not extend above the dwelling’s gutter line. Please refer to the SEPP legislation for roof height restrictions: https://legislation.nsw.gov.au/view/html/inforce/current/epi-2008-0572#sec.2.12")
            
            ## (??if the logic setting has too be attached roof? #Clause(j1)  no higher than 3m at its highest point above ground level (existing)
            if attributes["roof_height"] > 3:  
                issues.append("❌ Not Exempt for development as the roof’s highest point is over 3 meters above ground level. Please refer to the SEPP legislation for roof height restrictions: https://legislation.nsw.gov.au/view/html/inforce/current/epi-2008-0572#sec.2.12")

            if attributes["fascia_connection"] == "yes":
                if attributes["engineer_spec"] == "no":
                    issues.append("❌ Not Exempt for development as the fascia connection is not compliant with professional specifications. Please refer to the SEPP legislation for fascia connection restrictions: https://legislation.nsw.gov.au/view/html/inforce/current/epi-2008-0572#sec.2.12")
            
            ## (??if the logic setting has too be attached roof? #Clause (l)  be constructed or installed so that any roofwater is disposed of into an existing stormwater drainage system
            if attributes["stormwater"] == "no":
                issues.append("❌ Not Exempt for development as your roofwater does not dispose into an stormwater drainage system. Please refer to the SEPP legislation for stormwater disposal restrictions: https://legislation.nsw.gov.au/view/html/inforce/current/epi-2008-0572#sec.2.12")

    if attributes["drainage"] == "yes":
        issues.append("❌ Not Exempt for development as the proposed development interferes with existing drainage fixtures or flow paths. Please refer to the SEPP legislation for drainage restrictions: https://legislation.nsw.gov.au/view/html/inforce/current/epi-2008-0572#sec.2.12")

    if attributes["bushfire"] == "yes":
        if attributes["distance_dwelling"] < 5:
            if attributes["non_combustible"] == "no":
                issues.append("❌ Not Exempt for development as the bushfire material standards are not met. Please refer to the SEPP legislation for bushfire restrictions: https://legislation.nsw.gov.au/view/html/inforce/current/epi-2008-0572#sec.2.12")
...
def shed_check():
    ...
    if attributes["heritage"] == "yes":
        issues.append("❌ Not Exempt for development because the property is a heritage item. Please refer to the SEPP legislation for heritage item restrictions: https://legislation.nsw.gov.au/view/html/inforce/current/epi-2008-0572#sec.2.17")

    if attributes["foreshore"] == "yes":
        issues.append("❌ Not Exempt for development because the property is located in a foreshore area. Please refer to the SEPP legislation for foreshore area restrictions: https://legislation.nsw.gov.au/view/html/inforce/current/epi-2008-0572#sec.2.17")

    if attributes["sensitive_area"] == "yes":
        issues.append("❌ Not Exempt for development because the property is located in an environmentally sensitive area. Please refer to the SEPP legislation for environmentally sensitive area restrictions: https://legislation.nsw.gov.au/view/html/inforce/current/epi-2008-0572#sec.2.17")

    if attributes["zoning"] in ["R1", "R2", "R3", "R4"] and attributes ["area"] > 20:
        issues.append("❌ Not Exempt for development as the proposed structure's area exceeds the limit of 20m². Please refer to the SEPP legislation for area restrictions: https://legislation.nsw.gov.au/view/html/inforce/current/epi-2008-0572#sec.2.18")

    if attributes["zoning"] in ["R5", "RU1", "RU2", "RU3", "RU4", "RU6"] and attributes ["area"] > 50:
        issues.append("❌ Not Exempt for development as the proposed structure's area exceeds the limit of 50m². Please refer to the SEPP legislation for area restrictions: https://legislation.nsw.gov.au/view/html/inforce/current/epi-2008-0572#sec.2.18")

    if attributes["height"] > 3:
        issues.append("❌ Not Exempt for development as the proposed structure is higher than 3 m above ground level. Please refer to the SEPP legislation for height restrictions: https://legislation.nsw.gov.au/view/html/inforce/current/epi-2008-0572#sec.2.18")

    if attributes["zoning"] in ["R1", "R2", "R3", "R4"] and attributes["boundary_distance"] < 900:
        issues.append("❌ Not Exempt for development as the structure must be at least 900mm from the boundary. Please refer to the SEPP legislation for boundary distance restrictions: https://legislation.nsw.gov.au/view/html/inforce/current/epi-2008-0572#sec.2.18")
   
    if attributes["zoning"] in ["R5", "RU1", "RU2", "RU3", "RU4", "RU6"] and attributes["boundary_distance"] < 5000:
        issues.append("❌ Not Exempt for development as the structure must be at least 5000mm from the boundary. Please refer to the SEPP legislation for boundary distance restrictions: https://legislation.nsw.gov.au/view/html/inforce/current/epi-2008-0572#sec.2.18")

    if attributes["building_line"] == "no":
        if attributes["zoning"] not in ["RU1", "RU2", "RU3", "RU4", "RU6"]:
            issues.append("❌ Not Exempt for development as the structure must be behind the building line. Please refer to the SEPP legislation for building line restrictions: https://legislation.nsw.gov.au/view/html/inforce/current/epi-2008-0572#sec.2.18")

    if attributes["shipping_container"] == "yes":
        issues.append("❌ Not Exempt for development as shipping containers are not allowed. Please refer to the SEPP legislation for shipping container restrictions: https://legislation.nsw.gov.au/view/html/inforce/current/epi-2008-0572#sec.2.18")

    if attributes["stormwater"] == "no":
        issues.append("❌ Not Exempt for development your roofwater disposal may affect your neighbours. Please refer to the SEPP legislation for stormwater disposal restrictions: https://legislation.nsw.gov.au/view/html/inforce/current/epi-2008-0572#sec.2.18")

    if attributes["metal"] == "yes":
        if attributes["reflective"] == "no":
            issues.append("❌ Not Exempt for development as your metal components are not compliant. Please refer to the SEPP legislation for metal component restrictions: https://legislation.nsw.gov.au/view/html/inforce/current/epi-2008-0572#sec.2.18")

    if attributes["bushfire"] == "yes":
        if attributes["distance_dwelling"] < 5:
            if attributes["non_combustible"] == "no":
                issues.append("❌ Not Exempt for development as the bushfire material standards are not met. Please refer to the SEPP legislation for bushfire restrictions: https://legislation.nsw.gov.au/view/html/inforce/current/epi-2008-0572#sec.2.18")

    if attributes["adjacent_building"] == "yes":
        if attributes["interfere"] == "yes":
            issues.append("❌ Not Exempt for development as the structure interferes with building access or safety. Please refer to the SEPP legislation for adjacent building restrictions: https://legislation.nsw.gov.au/view/html/inforce/current/epi-2008-0572#sec.2.18")

    if attributes["habitable"] == "yes":
        issues.append("❌ Not Exempt for development because your proposed structure cannot be used as habitable buildings. Please refer to the SEPP legislation for habitable building restrictions: https://legislation.nsw.gov.au/view/html/inforce/current/epi-2008-0572#sec.2.18")

    if attributes["easement"] == "yes":
        issues.append("❌ Not Exempt for development as the proposed structure should be at least 1m from the easement. Please refer to the SEPP legislation for easement restrictions: https://legislation.nsw.gov.au/view/html/inforce/current/epi-2008-0572#sec.2.18")

    if attributes["services"] == "yes":
        issues.append("❌ Not Exempt for development as service connections are not allowed. Please refer to the SEPP legislation for service connection restrictions: https://legislation.nsw.gov.au/view/html/inforce/current/epi-2008-0572#sec.2.18")

    if attributes["existing_structures"] == "yes":
        issues.append("❌ Not Exempt for development as you already have two similar structures on your property. Please refer to the SEPP legislation for existing structure restrictions: https://legislation.nsw.gov.au/view/html/inforce/current/epi-2008-0572#sec.2.18")

    # Final return
    if issues:
        return "\n\n".join(issues)
    else:
        return (  "✅ Exempt Development: YYour structure passed the qualification test under SEPP legislation in this evaluation. "
                  "Please contact Albury City Council with your development details to confirm."
                  "please refer to the SEPP legislation: https://legislation.nsw.gov.au/view/html/inforce/current/epi-2008-0572#pt.2-div.1-sdiv.6"""
        )
